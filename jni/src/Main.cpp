...
struct SnapshotBuffer {
    std::unordered_map<uintptr_t, EntitySnapshot> entities;
    float 矩阵数组[16];
    float 自身坐标[3];
    int 数组数量 = 0;
    uintptr_t 数组指针 = 0;
...
SnapshotBuffer g_buffers[3];
std::atomic<int> g_writeIdx{0};
std::atomic<int> g_renderIdx{1};
std::atomic<bool> g_readerRunning{false};
...
void ReaderThread() {
    ...
    while (g_readerRunning) {
        int wIdx = g_writeIdx.load(std::memory_order_relaxed);
        SnapshotBuffer& writeBuf = g_buffers[wIdx];
        ...
        uintptr_t 自身地址 = Driver.Read<uintptr_t>(Driver.Read<uintptr_t>(Driver.Read<uintptr_t>(定义.UE4+自身头)+Uworld)+自身指针);
        
        float selfCheck = Driver.Read<float>(自身地址 + 过滤物资指针);
        if (selfCheck == 479.5) {
            if (Cloud.广角) {
                uintptr_t scopeFovPtr = Driver.Read<uintptr_t>(自身地址 + 开镜视角组件) + 广角指针;
                uintptr_t firstFovPtr = Driver.Read<uintptr_t>(自身地址 + 一称视角组件) + 广角指针;
                uintptr_t thirdFovPtr = Driver.Read<uintptr_t>(自身地址 + 三称视角组件) + 广角指针;
                uintptr_t camFovPtr = Driver.Read<uintptr_t>(自身地址 + 相机视角组件) + 广角指针;
                Driver.Write<float>(scopeFovPtr, Cloud.开镜广角大小);
                Driver.Write<float>(firstFovPtr, Cloud.一称广角大小);
                Driver.Write<float>(thirdFovPtr, Cloud.三称广角大小);
                Driver.Write<float>(camFovPtr, Cloud.相机广角大小);
            }
            uintptr_t weaponAddr = Driver.Read<uintptr_t>(Driver.Read<uintptr_t>(Driver.Read<uintptr_t>(自身地址 + 武器经理) + 当前武器) + 武器实体);
            if (weaponAddr) {
                int wType = Driver.Read<unsigned char>(weaponAddr + 武器类型指针);
                if (wType != 0) {
                    if (Cloud.聚点)
                        Driver.Write<float>(Driver.Read<uintptr_t>(weaponAddr + 聚点指针), 0);
                    if (Cloud.无后)
                        Driver.Write<float>(Driver.Read<uintptr_t>(weaponAddr + 无后指针), 0.15);
                }
            }
        }

        uintptr_t 矩阵地址 = Driver.Read<uintptr_t>(Driver.Read<uintptr_t>(定义.UE4+矩阵头)+0x20)+0x270;
        Driver.Read(矩阵地址, writeBuf.矩阵数组, 16*4);
        
        uintptr_t 数组地址 = 数组解密(Driver.Read<uintptr_t>(Driver.Read<uintptr_t>(Driver.Read<uintptr_t>(定义.UE4+自身头)+Uworld)+Ulevel),0xA0,0x448);
        writeBuf.数组数量 = Driver.Read<int>(数组地址+0x8);
        writeBuf.数组指针 = Driver.Read<uintptr_t>(数组地址);
        
        Driver.Read(Driver.Read<uintptr_t>(自身地址+Mesh指针)+坐标指针, writeBuf.自身坐标, 12);
        
        writeBuf.entities.clear();
        
        if (writeBuf.数组数量 >= 10000 || writeBuf.数组数量 < 0) {
            usleep(5000);
            continue;
        }
        
        for (int i = 0; i <= writeBuf.数组数量; i++) {
            uintptr_t 对象 = Driver.Read<uintptr_t>(writeBuf.数组指针+0x8*i);
            
            float 过滤物资 = Driver.Read<float>(对象+过滤物资指针);
            int 过滤载具 = Driver.Read<int>(对象+过滤载具指针);
            
            if (过滤载具 == 20000 || 过滤物资 == 479.5) {
                EntitySnapshot snap;
                snap.valid = true;
                
                uintptr_t Mesh = Driver.Read<uintptr_t>(对象+Mesh指针);
                uintptr_t 根 = Driver.Read<uintptr_t>(对象+根指针);
                
                Vector_3D rawCoord = Driver.Read<Vector_3D>(Mesh+坐标指针);
                
                if (Cloud.Decryption && 过滤物资 == 479.5) {
                    Vector_3D decrypted = Decrypt_Location({rawCoord.x, rawCoord.y, rawCoord.z}, 对象);
                    snap.坐标 = 三维(decrypted.x, decrypted.y, decrypted.z);
                } else if (过滤物资 == 479.5) {
                    snap.坐标 = 三维(rawCoord.x, rawCoord.y, rawCoord.z);
                } else {
                    Vector_3D rootCoord = Driver.Read<Vector_3D>(根+坐标指针);
                    snap.坐标 = 三维(rootCoord.x, rootCoord.y, rootCoord.z);
                }
                
                Driver.Read(根+向量指针, &snap.向量, 12);
                uintptr_t 当前载具 = Driver.Read<uintptr_t>(对象+当前载具指针);
                if (当前载具) {
                    Driver.Read(当前载具+载具向量指针, &snap.载具向量, 12);
                } else {
                    snap.载具向量 = 三维(0,0,0);
                }
                snap.敌人视角 = Driver.Read<float>(根+旋转指针);

                float dx = snap.坐标.x - writeBuf.自身坐标[0];
                float dy = snap.坐标.y - writeBuf.自身坐标[1];
                float dz = snap.坐标.z - writeBuf.自身坐标[2];
                snap.Distance = sqrt(dx*dx + dy*dy + dz*dz) / 100;
                
                writeBuf.entities[对象] = snap;
            }
        }
        
        writeBuf.ready = true;
        
        int old_render = g_renderIdx.load(std::memory_order_relaxed);
        g_renderIdx.store(wIdx, std::memory_order_release);
        g_writeIdx.store(old_render, std::memory_order_release);
...
int main() {
    ...
    pthread_create(&reader_tid, NULL, (void*(*)(void*))ReaderThread, NULL);
    usleep(2000);
...
void 绘制函数(ImDrawList *绘制) {
    int rIdx = g_renderIdx.load(std::memory_order_acquire);
    SnapshotBuffer& snapBuf = g_buffers[rIdx];
    if (!snapBuf.ready) return;
    ...
    定义.自身坐标={snapBuf.自身坐标[0],snapBuf.自身坐标[1],snapBuf.自身坐标[2]};
    定义.数组数量=snapBuf.数组数量;
    定义.数组指针=snapBuf.数组指针;
    ...
    memcpy(定义.矩阵数组, snapBuf.矩阵数组, sizeof(定义.矩阵数组));
    ...
    定义.有效腰射距离=Cloud.腰射距离;
    定义.有效开镜距离=Cloud.开镜距离;
    定义.有效自瞄范围=Cloud.自瞄范围;
    定义.有效腰射速度=Cloud.腰射速度;
    定义.有效开镜速度=Cloud.开镜速度;
    定义.有效腰射压枪=Cloud.腰射压枪;
    定义.有效开镜压枪=Cloud.开镜压枪;
    定义.有效预判=Cloud.预判;
    ...
    auto it = snapBuf.entities.find(定义.对象);
    if (it != snapBuf.entities.end() && it->second.valid) {
        定义.坐标 = it->second.坐标;
        定义.向量 = it->second.向量;
        定义.载具向量 = it->second.载具向量;
        定义.敌人视角 = it->second.敌人视角;
        定义.Distance = it->second.Distance;
    } else {
...
if(Cloud.Skeleton){
ImColor 骨骼颜色 = IM_COL32(255,255,255,200);
绘制->AddCircle(ImVec2(获取二维骨骼(定义.坐标,定义.Human,定义.Bone,6).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,6).y-(获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y-获取二维骨骼(定义.坐标,定义.Human,定义.Bone,6).y)),(获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y-获取二维骨骼(定义.坐标,定义.Human,定义.Bone,6).y)*2,骨骼颜色,200,2.5f);
if(!定义.人机判断){
if(定义.骨骼数量==65){
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y+7},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,1).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,1).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,34).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,34).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,13).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,13).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,34).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,34).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,35).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,35).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,13).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,13).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,35).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,35).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,37).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,37).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,16).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,16).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,58).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,58).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,58).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,58).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).y},骨骼颜色,{2.5f});
}else if(定义.骨骼数量==67){
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y+7},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,1).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,1).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,34).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,34).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,13).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,13).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y},骨骼颜色,{2.5f});
...
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,35).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,35).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,37).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,37).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,16).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,16).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,56).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,56).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,56).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,56).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,60).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,60).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,61).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,61).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,60).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,60).y},骨骼颜色,{2.5f});
}
...
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,58).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,58).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,58).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,58).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).y},骨骼颜色,{2.5f});
...
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,56).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,56).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,57).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,56).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,56).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,59).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,60).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,60).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,61).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,61).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,60).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,60).y},骨骼颜色,{2.5f});
}else if(定义.人机判断&&定义.骨骼数量==29){
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y+7},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,1).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,1).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,8).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,8).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,14).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,15).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,15).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,15).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,15).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,16).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,16).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,8).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,8).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,9).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,9).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,9).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,9).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,10).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,10).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,22).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,22).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,19).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,19).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,22).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,22).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,23).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,23).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,23).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,23).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,24).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,24).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,19).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,19).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,20).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,20).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,21).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,21).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,20).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,20).y},骨骼颜色,{2.5f});
}else if(定义.人机判断&&定义.骨骼数量==61){
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y+7},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,1).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,1).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,29).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,29).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,8).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,8).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,5).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,29).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,29).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,30).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,30).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,30).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,30).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,31).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,31).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,8).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,8).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,9).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,9).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,9).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,9).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,10).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,10).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,49).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,49).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,49).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,49).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,50).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,50).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,51).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,51).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,50).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,50).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,53).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).y},骨骼颜色,{2.5f});
绘制->AddLine({获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,54).y},{获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).x,获取二维骨骼(定义.坐标,定义.Human,定义.Bone,55).y},骨骼颜色,{2.5f});
}
}
...
if(Cloud.自瞄){
    // g_AimMode is shared with Menu_Ui.hpp — set via Combo there (BUG-24 fix)
    ...
    if (g_AimMode == 1) {
        ...
        if (Preset && Preset->Enabled) {
            定义.有效腰射距离 = Preset->HipfireDistance;
            定义.有效开镜距离 = Preset->ScopeDistance;
            定义.有效自瞄范围 = Preset->FOVRange;
            定义.有效腰射速度 = Preset->HipfireSpeed;
            定义.有效开镜速度 = Preset->ScopeSpeed;
            定义.有效腰射压枪 = Preset->HipfireRecoil;
            定义.有效开镜压枪 = Preset->ScopeRecoil;
            定义.有效预判 = Preset->Prediction;
        } else {
            定义.有效腰射距离 = Cloud.腰射距离;
            定义.有效开镜距离 = Cloud.开镜距离;
            定义.有效自瞄范围 = Cloud.自瞄范围;
    ...
    if(定义.开镜 && 定义.Distance > 定义.有效开镜距离 || !定义.开镜 && 定义.Distance > 定义.有效腰射距离)
        continue;
    ...
    if(!定义.开镜) 定义.速度 = 定义.有效腰射速度;
    else 定义.速度 = 定义.有效开镜速度;
    ...
    if(定义.准星 <= 定义.有效自瞄范围) {
        ...
        if(过滤变量 < 定义.最小距离) {
            ...
            if(定义.开火 && !识别喷子(定义.武器类型) && !识别狙击(定义.武器类型)) {
                if(!定义.开镜) 压枪值 = 定义.有效腰射压枪 * 定义.Distance / 100 * 识别压枪(定义.手持武器) * 动作压枪(定义.自身状态);
                else 压枪值 = 定义.有效开镜压枪 * 定义.Distance / 100 * 矩阵转换(定义.坐标, 定义.矩阵数组).高度 * 识别压枪(定义.手持武器) * 动作压枪(定义.自身状态);
            } else {
                压枪值 = 0;
            }
            if(Cloud.部位 == 0) {
                定义.瞄准.x = 获取二维预判骨骼(定义.坐标, 定义.Human, 定义.Bone, 定义.向量, 定义.载具向量, 定义.有效预判, Cloud.载具预判, 定义.Distance, 定义.子弹速度, 定义.上车, 6).x;
                定义.瞄准.y = 获取二维预判骨骼(定义.坐标, 定义.Human, 定义.Bone, 定义.向量, 定义.载具向量, 定义.有效预判, Cloud.载具预判, 定义.Distance, 定义.子弹速度, 定义.上车, 6).y + 压枪值;
            } else {
                定义.瞄准.x = 获取二维预判骨骼(定义.坐标, 定义.Human, 定义.Bone, 定义.向量, 定义.载具向量, 定义.有效预判, Cloud.载具预判, 定义.Distance, 定义.子弹速度, 定义.上车, 4).x;
                定义.瞄准.y = 获取二维预判骨骼(定义.坐标, 定义.Human, 定义.Bone, 定义.向量, 定义.载具向量, 定义.有效预判, Cloud.载具预判, 定义.Distance, 定义.子弹速度, 定义.上车, 4).y + 压枪值;
            }
        }
    }
}}}}
...
float 范围;
if(!Cloud.DynamicFOV)范围=定义.有效自瞄范围; else if((定义.开火||Cloud.喷子&&识别喷子(定义.武器类型)||Cloud.狙击&&识别狙击(定义.武器类型)&&定义.开镜)&&定义.自瞄人数>0)范围=定义.最小距离;else 范围=定义.有效自瞄范围;
...
定义.自瞄总数=定义.自瞄人数;
    ...
    if (MainWindow.x < 100 && MainWindow.y < 100 && !SmallMenuToggle) {
        ...
        绘制->AddText(NULL, 40, ImVec2(SmallMenuPosition.x + 100 - Size.x / 2, SmallMenuPosition.y + 37.5 - Size.y / 2), ImColor(139, 0, 0, 255), "NovaX");
    }
return;
}